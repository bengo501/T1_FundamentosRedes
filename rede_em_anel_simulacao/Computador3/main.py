...
def ver_logs():
    ...
    print("="*50)
    try:
        with open(f"logs_Computer3.log", "r", encoding="utf-8") as f:
...
def enviar_mensagem_com_erro():
    ...
    # Força um erro de CRC modificando a mensagem
    ...
    with mutex:
        ...
        registrar_log(f"[{apelido}] ⚠️ Status: Aguardando envio com erro forçado")
        print(f"\n[Fila] Mensagem com erro forçado adicionada.")
        input("\nPressione Enter para continuar...")
            ...
            if mensagem.startswith("DISCOVER:"):  # Mensagem de descoberta
                ...
                if origem == apelido:
                    if controle == "naoexiste":
                        ...
                        if controle == "NACK":
                            registrar_log(f"[{apelido}] ⚠️ NACK recebido - Erro detectado no destino")
                            ...
                            if fila_mensagens:
                                if controle == "ACK" or controle == "naoexiste":
                                    ...
                                    if not reenviado:
                                        ...
                                        registrar_log(f"[{apelido}] ⚠️ A mensagem não pôde ser entregue após duas tentativas")
        ...
        try:
            with mutex:
                ...
                if gerar_token and time.time() - ultima_passagem_token > tempo_maximo_token:
                    registrar_log(f"[{apelido}] ⚠️ TIMEOUT! Regenerando token...")
                    ...
                    if fila_mensagens:
                        ...
                        try:
                            ...
                            if "ERRO_FORÇADO" in mensagem_pronta:
                                ...
                                registrar_log(f"[{apelido}] ⚠️ A mensagem será tratada pelo destinatário")
                            else:
                            ...
                            registrar_log(f"[{apelido}] ⚠️ Mensagem mantida na fila para nova tentativa")
